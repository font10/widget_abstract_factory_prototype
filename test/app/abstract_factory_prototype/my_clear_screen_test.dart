...
class MockUIFactory implements UIFactory {
  ...
  IAppDialog createDialog({
    ...
    dialogCreated = true;
    return MaterialAppDialog(
      title: 'Mock Dialog',
      content: 'This is a mock dialog',
      actions: [],
    );
  }
}
...
void main() {
  testWidgets('La pantalla debe solicitar componentes a la fábrica inyectada', (
    WidgetTester tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: MyCleanScreen(uiFactory: mockFactory)),
    );
  ...
  testWidgets('La pantalla solicita el TextField con el placeholder correcto', (
    WidgetTester tester,
  ) async {
    // Arrange
    final mockFactory = MockUIFactory();
    ...
    // Act
    final textfield = mockFactory.createTextField(
      placeholder: 'Ingrese su nombre',
      controller: TextEditingController(),
    );

    textfield.controller?.text =
        'Test'; // Simulamos que el usuario ingresa texto
    // Assert
  ...
  test(
    'El boton creado por la fabrica debe abrir el dialogo con el mensaje correcto',
    () {
      // Arrange
      final mockFactory = MockUIFactory();
      ...
      final dialog = mockFactory.createDialog(
        title: 'Error',
        content:
            'Este es un mensaje de error genérico, sin importar la plataforma.',
        actions: [],
      );
