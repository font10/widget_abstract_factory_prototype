...
class CupertinoUIFactory implements UIFactory {
  ...
  final IAppButton _buttonPrototype = const CupertinoAppButton(text: 'prototypeButton');
  final IAppTextField _textFieldPrototype = const CupertinoAppTextField(placeholder: 'prototypeTextField');
  final IAppDialog _dialogPrototype = const CupertinoAppDialog(title: 'prototypeDialog', content: 'This is a prototype dialog', actions: []);
  ...
  IAppDialog createDialog({required String title, required String content, required List<Widget> actions}) {
    return _dialogPrototype.clone(title: title, content: content, actions: actions);
  }
}